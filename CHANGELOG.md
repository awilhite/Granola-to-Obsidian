...
- **Template generation safeguards**: The plugin verifies persisted structured content, attempts best-effort soft deletion of the newly created panel after generation, verification, or refresh failure, protects empty matching panels with reliable timestamps until they are at least 10 minutes old, reports protected work as `template-deferred`, and continues syncing current content after a non-blocking template failure. Cleanup is not guaranteed and does not use a final emptiness check.
