...
- **🔄 Quieter routine sync checks**: Auto-sync now shows a simple `Syncing` status during lightweight per-note scans. For the `changed` behavior, Template Management checks current notes before the up-to-date skip, uses embedded populated panels as a cheap ready path, and limits native generation to one new panel per sync run; `never` still performs no work for existing notes.
...
- **Template-generation compatibility**: Every Template Management request now sends `auto: false`, including scheduled syncs. The configured plugin scheduler remains automatic; live validation found `auto: true` returns HTTP 409 for this flow. Generic orchestration failures expose only a numeric HTTP status or `unknown`.
- **Template generation safeguards**: The plugin verifies persisted structured content, preserves verified populated panels when the final document refresh fails, and deletes a failed plugin-created panel only after a private re-fetch proves that exact panel is active and empty. Ambiguous or populated panels are preserved, protected or budget-limited work is reported as `template-deferred`, and normal sync continues after non-blocking template failures.
- **🩹 Malformed summary normalization**: Collapsed one-line Granola summaries remain a narrow read-time repair for selected-template, enhanced-notes, and last-viewed-panel Markdown matching the existing malformed-summary predicates; this is not a fallback writeback path
