...
- **Template-generation compatibility**: Every Template Management request now sends `auto: false`, including scheduled syncs. The five-minute plugin schedule remains automatic; live validation found `auto: true` returns HTTP 409 for this flow. Generic orchestration failures expose only a numeric HTTP status or `unknown`.
